...
/// 一个泛型的类，通过let instance = Box(value: a usable value)定义后 能通过 instance.value直接拿到value，喵神通过这个做了Kingfisher加载小菊花及其类型的setter getter
class Box<T> {
    let value: T
    init(value: T) {
        self.value = value
...
/**
 String+MD5 顾名思义就是一个MD5加密的工具，至于🐱神为什么会给字符串做一个代理的Struct，我的猜想是想做一个统一，e.g：imageKey.kf.md5、button.kf.setImage:forstate:、imageView.kf.setImage:，体现Kingfisher的功能点，如果你想把它单独抽出来，只需要去掉字符串代理部分即可
 */

// MARK: - MD5抽离Kingfisher扩展，so easy，当时你想到别人调用的时候无时无刻都要写到你的名字，这也是一种美滋滋的体验吧，我等小码农心态😷。
extension String {
...
// MARK: - 最后是一个线程安全的扩展，曾在Yep也看到类似的代码，不过判断方式不同，而Yep的方式更加完善 
// TODO: //具体解释
